...
contract Ownable is Context {
    ...
    constructor() {
        _owner = 0x754ddA1A30F4e2d8aDDcDf7A254298e365224273;
...
contract SeekCoin is Context, IBEP20, Ownable {
    ...
    string private _name = "SeekCoin";
    string private _symbol = "SEEK";
    ...
    uint256 supplylimitremain = 350_000_000 ether;
    ...
    uint256 public _maxTxAmount = 100_000_000 ether;
    ...
    constructor() {
        ...
        _isExcludedFromFee[seekrewardContractAddress] = true;

        IDEXRouter _DEXRouter = IDEXRouter(
            0x10ED43C718714eb63d5aA57B78B54704E256024E // no change required
        ); //testnet and mainnet
        // Create a DEX pair for this new token
        address pair1 = IDEXFactory(_DEXRouter.factory()).createPair(
            address(this),
            _DEXRouter.WETH()
        );
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = tAmount
            .mul(_holderFee.add(_burnFee).add(_seekrewardFee))
            .div(100);
        return percentage;
    }
    ...
    function setSeekRewardFee(uint256 seekrewardFee) public onlyOwner {
        _seekrewardFee = seekrewardFee;
    }

    function setSeekRewardAddress(address _address) public onlyOwner {
        seekrewardContractAddress = _address;
    }
    ...
    function _takeSeekFee(uint256 tAmount) private {
        uint256 tSeekFee = tAmount.mul(_seekrewardFee).div(100);
        uint256 rSeekFee = tSeekFee.mul(_getRate());
        _rOwned[seekrewardContractAddress] = _rOwned[address(this)].add(
            rSeekFee
        );
        if (_isExcluded[seekrewardContractAddress])
            _tOwned[seekrewardContractAddress] = _tOwned[
                seekrewardContractAddress
            ].add(tSeekFee);
        emit Transfer(_msgSender(), seekrewardContractAddress, tSeekFee);
    }
    ...
    function removeAllFee() private {
        if (_holderFee == 0) return;

        _previousTaxFee = _holderFee;

        _previousburnFee = _burnFee;

        _previousseekFee = _seekrewardFee;
    ...
    function restoreAllFee() private {
        ...
        _seekrewardFee = _previousseekFee;
    ...
    function _transfer(
        ...
        require(amount > 0);
        if (from != owner() && to != owner()) require(amount <= _maxTxAmount);
        if (!tradingEnabled && from != owner()) {
            require(!isSwap[to], "can not add liquidity before owner");
        }

        if (balanceOf(burnAddress) >= supplylimitremain) {
            _burnFee = 0;
            _previousburnFee = 0;
        }
